...
const MainSmtrack = () => {
  ...
  const { cookieDecode, tokenDecode } = useSelector((state: RootState) => state.utils)
  ...
  const fetchUserProfile = async () => {
    if (id) {
      try {
        const response = await axiosInstance.get<responseType<UserProfileType>>(`${import.meta.env.VITE_APP_NODE_ENV === "development" ? import.meta.env.VITE_APP_AUTH : ''}/auth/user/${id}`)
